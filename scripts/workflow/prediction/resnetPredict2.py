...
def main(dbPath, proteinListPath, outputDirPath):
    ...
    for chopLayer in ["activation_46","activation_43","activation_40","activation_37", "activation_34", "activation_31", "activation_28", "activation_25", "activation_22", "activation_19", "activation_16", "activation_13", "activation_10"]:
        print("Chop Layer: ", chopLayer)
        
        #Chop the model at a layer of our choice.
        chopLayerOutput = resnet_model.get_layer(chopLayer).output;
        pooledOutput = GlobalAveragePooling2D()(chopLayerOutput)
        choppedModel = Model(inputs=resnet_model.input, outputs=pooledOutput)
        resnet_model = choppedModel
        ...
        try:
            ...
            with open("output-"+chopLayer+".csv", "w") as csvfile:
                ...
                for row in proteinRowIter(dbConn, proteinListPath):
                    ...
                    if flatSize is None:
                        flatSize = 1
                        for dimension in predictions.shape:
                            flatSize *= dimension
                    flatPreds = np.reshape(predictions,flatSize)
                    for val in row:
